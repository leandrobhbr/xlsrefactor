...
class ExcelController extends Controller
{
    public function pbh(){
        $pbh = new InPBH();
        Excel::import($pbh, "pbh.xlsx");
        $dataArray = $pbh->getArrayLines();
        return Excel::download(new ExportSessionArray($dataArray), 'pbh2409.csv', \Maatwebsite\Excel\Excel::CSV);
    }

    public function discentes(){
        $discentes = new InAlunos();
        Excel::import($discentes, "discentes.xls");
        $dataArray = $discentes->getArrayLines();
        return Excel::download(new ExportSessionArray($dataArray), 'discentes2209.csv', \Maatwebsite\Excel\Excel::CSV);
    }

    public function posdoutoral(){
        $posdoutoral = new InAlunos();
        Excel::import($posdoutoral, "posdoutoral.xls");
        $dataArray = $posdoutoral->getArrayLines();
        return Excel::download(new ExportSessionArray($dataArray), 'posdoutoral2209.csv', \Maatwebsite\Excel\Excel::CSV);
    }
